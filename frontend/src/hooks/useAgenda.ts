...
import type { Agenda, CreateAgendaDTO, UpdateAgendaDTO } from '../services/agendaService';
...
export const useAgenda = () => {
  ...
  const update = useCallback(async (id: string, data: UpdateAgendaDTO) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const marcarRenovou = useCallback(async (id: string, data?: any) => {
    setLoading(true);
    setError(null);
    try {
      const updated = await agendaService.marcarRenovou(id, data);
      setAgenda(prev => prev.map(a => a.id === id ? updated : a));
      return updated;
    } catch (err) {
  ...
  const marcarCancelou = useCallback(async (id: string, motivo?: string) => {
    setLoading(true);
    setError(null);
    try {
      const updated = await agendaService.marcarCancelou(id, motivo);
      setAgenda(prev => prev.map(a => a.id === id ? updated : a));
      return updated;
    } catch (err) {
