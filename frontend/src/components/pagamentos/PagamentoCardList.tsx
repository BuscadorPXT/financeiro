...
interface PagamentoCardListProps {
  ...
  sortKey: keyof Pagamento;
  sortOrder: 'asc' | 'desc';
  onSort: (key: keyof Pagamento) => void;
}

const PagamentoCardList: React.FC<PagamentoCardListProps> = ({
  pagamentos,
  usuarios,
  onEdit,
  onDelete,
  ...
  return (
    <div>
      ...
      <div className="bg-[var(--bg-secondary)] px-4 py-3 border-b border-[var(--border-color)] flex gap-2 flex-wrap rounded-t-lg">
        ...
        <button
          onClick={() => onSort('dataPagto')}
          className="text-sm text-blue-600 hover:underline font-medium"
        >
          Data {sortKey === 'dataPagto' && (sortOrder === 'asc' ? '↑' : '↓')}
        </button>
        <button
          onClick={() => onSort('mesPagto')}
          className="text-sm text-blue-600 hover:underline font-medium"
        >
      ...
      {paginatedData.length === 0 ? (
        <div className="text-center py-12 bg-[var(--bg-primary)] rounded-b-lg shadow">
          <p className="text-[var(--text-secondary)] text-lg">Nenhum pagamento encontrado</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4 p-4 bg-[var(--bg-secondary)] rounded-b-lg">
          {paginatedData.map((pagamento) => (
            <PagamentoCard
              key={pagamento.id}
              pagamento={pagamento}
              usuario={getUsuario(pagamento.usuarioId)}
              onEdit={onEdit}
              onDelete={onDelete}
            />
          ))}
...
export default PagamentoCardList;
